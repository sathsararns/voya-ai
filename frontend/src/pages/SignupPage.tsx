...
export function SignupPage() {
  ...
  const onSubmit = async (values: SignupFormValues) => {
    setFormError(null)
    try {
      // Signup signs the account in immediately — no email-verification
      // step to send them through first.
  ...
  return (
    <AuthLayout title="Create your account" subtitle="Plan trips, save itineraries, and pick up where you left off.">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="mb-1.5 block text-[13px] font-medium text-muted" htmlFor="name">
            Name
          </label>
        ...
        <div>
          <label className="mb-1.5 block text-[13px] font-medium text-muted" htmlFor="password">
            Password
          </label>
